...
# Load config
with open("config copy.yaml", "r") as f:
...
def evaluate_model(model, x_pos, env_params, n_eval_episodes=10, deterministic=True):
    ...
    env = VecNormalize.load("logs_copy/vecnormalize.pkl", env) if os.path.exists("logs_copy/vecnormalize.pkl") else env
    env.training = False
...
def main():
    ...
    eval_cb = EvalCallback(
        eval_env,
        best_model_save_path="logs_copy/best_model",
        log_path="logs_copy/eval",
        eval_freq=2048,
        deterministic=True,
        n_eval_episodes=5,
    )
    ...
    tensorboard_log_dir = os.path.join(
        config['ppo'].get('tensorboard_log', './ppo_assembly_tensorboard_copy/'),
        f"run_{run_id}"
    )
    ...
    for iteration in range(total_iterations):
        ...
        train_envs.save("logs_copy/vecnormalize.pkl")
        ...
        model.save(f"ppo_checkpoints_copy/ppo_stochastic_env_iter{iteration+1}.zip")

    # Save final model
    model.save(config.get('output_model_path', 'ppo_checkpoints_copy/ppo_stochastic_env.zip'))
    train_envs.save("logs_copy/vecnormalize.pkl")
