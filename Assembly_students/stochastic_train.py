...
def evaluate_model(model, x_pos, env_params, n_eval_episodes=10, deterministic=True):
    ...
    env = VecNormalize.load("logs/vecnormalize.pkl", env) if os.path.exists("logs/vecnormalize.pkl") else env
    env.training = False
...
def main():
    ...
    tensorboard_log_dir = os.path.join(
        config['ppo'].get('tensorboard_log', './ppo_assembly_tensorboard/'),
        f"run_{run_id}"
    )
    ...
    for iteration in range(total_iterations):
        ...
        model.save(f"ppo_checkpoints/ppo_stochastic_env_iter{iteration+1}.zip")

    # Save final model
    model.save(config.get('output_model_path', 'ppo_checkpoints/ppo_stochastic_env.zip'))
    train_envs.save("logs/vecnormalize.pkl")
